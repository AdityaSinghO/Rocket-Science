...
def simulate_flight(water_vol_fraction=0.33, pressure_psi=60, launch_angle=85):
    ...
    mass_air = (P_initial * V_air) / (R_gas * T_initial)
    ...
    while y >= 0:
        ...
        if V_water > 0:  # Phase 1: Water
            ...
            if (P_atm / P_current) < crit_ratio:  # Choked
                ...
                pressure_term = np.maximum(0, (P_current / P_atm) ** ((gamma - 1) / gamma) - 1)
            ...
            if mass_air > 0:
                ...
                T_current = T_initial * (rho_current / (mass_air / bottle_volume)) ** (gamma - 1)
                P_current = rho_current * R_gas * T_current
            else:
                P_current = P_atm

        else:  # Phase 3: Coast / Descent
            thrust_force = 0.0
            dm_dt = 0.0
...
if __name__ == "__main__":
    ...
    ax2.set_title("2. Vertical Velocity vs Time")
    ...
    # --- Optimization: water fill fraction vs max altitude ---
    results = []
    frac = []
    for water_fraction in range(1, 60):
        frac.append(water_fraction * 0.01)
        sim_data = simulate_flight(water_fraction * 0.01)
        results.append(max(sim_data['y']))

    plt.figure(figsize=(8, 5))
    plt.plot(frac, results, '-ro')
    plt.title("Optimization: Water Fill Ratio vs Altitude")
    ...
    # --- Optimization: launch angle vs max altitude ---
    results = []
    ang = []
    ...
    print("Saved: out/flight_profile.png, out/water_fraction_sweep.png, out/launch_angle_sweep.png")
